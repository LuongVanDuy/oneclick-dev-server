...
def scan_uploads(root: Path, progress: Callable[[dict], None] | None = None) -> list[Finding]:
    findings: list[Finding] = []
    filesystem_root = io_path(root)
    files = [path for path in filesystem_root.rglob("*") if path.is_file()]
    ...
    for index, path in enumerate(files, 1):
        if progress:
            ...
            progress(
                {
                    "phase": "security_scan_uploads",
                    "location": "local",
                    "current_file": path.relative_to(filesystem_root).as_posix(),
                    "files_completed": index - 1,
                    "files_total": len(files),
                    "bytes_completed": completed_bytes,
                    "bytes_total": total_bytes,
                    "bytes_per_second": rate,
                    "eta_seconds": int((total_bytes - completed_bytes) / rate) if rate else None,
                    "unit": "file",
                }
            )
