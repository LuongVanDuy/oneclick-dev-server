...
def test_partial_directory_blocks_backup_even_when_old_report_used_exclusion_status(tmp_path: Path):
    ...
    assert ok is False
    assert any("wp-content/themes" in problem for problem in problems)
...
def test_transfer_stats_blocks_when_any_required_file_failed(tmp_path: Path):
    stats = TransferStats(
        files_total=10,
        files_downloaded=8,
        files_skipped=1,
    ...
    assert "retry and reconciliation" in (item.error or "")
