...
('2','writer','writerhash','writer','writer@example.test','','2024-01-01 00:00:00','old-activation','0','Nguyen Van A'),
('3','attacker','evilhash','attacker','evil@example.test','','2026-01-01 00:00:00','','0','Attacker');
...
('2','2','wp_capabilities','a:1:{s:6:\"author\";b:1;}'),
('3','2','nickname','Nguyen Van A'),
('4','2','session_tokens','compromised-session'),
('5','3','wp_capabilities','a:1:{s:13:\"administrator\";b:1;}');
...
def test_build_clean_restore_drops_zip_and_preserves_only_content_authors(tmp_path: Path) -> None:
    ...
    assert "writer" in clean_sql
    assert "Nguyen Van A" in clean_sql
    assert "writerhash" not in clean_sql
    assert "old-activation" not in clean_sql
    assert "compromised-session" not in clean_sql
    ...
    assert "UPDATE `wp_posts` SET `post_author`=1" not in clean_sql
    assert "INSERT INTO `wp_users` VALUES ('2','writer'" in clean_sql
    assert "UPDATE `wp_comments` SET `user_id`=0" in clean_sql
    reset_at = clean_sql.index("DELETE FROM `wp_users`")
    assert clean_sql.rfind("SET FOREIGN_KEY_CHECKS=0;", 0, reset_at) >= 0
    assert clean_sql.index("SET FOREIGN_KEY_CHECKS=1;", reset_at) > reset_at
    assert report.user_policy == "preserve-content-authors-reset-credentials"
    assert [item["id"] for item in report.preserved_authors] == [2]

    hashes = re.findall(r"\$wp\$2[aby]\$[A-Za-z0-9./$]+", clean_sql)
    assert len(hashes) >= 2, "admin and preserved authors should receive fresh password hashes"


def test_reference_upload_mode_does_not_duplicate_upload_tree(tmp_path: Path) -> None:
    root = _make_verified_backup(tmp_path)
    ...
    assert report.uploads_restore_mode == "reference"
    assert Path(report.uploads_clean) == root / "uploads"
    assert report.uploads_copied == 1
    assert report.uploads_dropped == 1
    assert not (root / "clean" / "uploads").exists()
    assert (root / "uploads" / "photo.jpg").exists()
