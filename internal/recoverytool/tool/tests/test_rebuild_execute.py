...
def test_database_import_bridge_uses_header_token_not_query_string():
    ...
    assert "SET @saved_cs_client = @@character_set_client" in bridge
    assert "SET @OLD_SQL_MODE = @@sql_mode" in bridge
    assert "SET SESSION FOREIGN_KEY_CHECKS = 0" in bridge
    assert "$statementToRun = 'SET SESSION FOREIGN_KEY_CHECKS = 0'" in bridge
    assert "FOREIGN_KEY_CHECKS\\s*=\\s*1" in bridge
