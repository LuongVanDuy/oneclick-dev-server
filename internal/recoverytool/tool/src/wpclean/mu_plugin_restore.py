...
from .mu_plugin_policy import ALLOWED_MU_PLUGIN_COMPONENT_KINDS, is_allowed_mu_plugin_component
...
def scan_mu_plugins(backup_root: Path) -> MuPluginStageReport:
    ...
    for component_path in components:
        ...
        if not is_allowed_mu_plugin_component(component.name):
            component.files_total = len(_component_files(component_path))
            component.blocked = True
            component.unreadable_files.append(
                "KHÔNG NẰM TRONG ALLOWLIST MU-PLUGIN; KHÔNG ĐƯỢC RESTORE"
            )
            report.blocked_components += 1
            report.components.append(component)
            continue
        expected_kind = ALLOWED_MU_PLUGIN_COMPONENT_KINDS[component.name.casefold()]
        if component.kind != expected_kind:
            component.files_total = len(_component_files(component_path))
            component.blocked = True
            component.unreadable_files.append(
                f"SAI LOẠI MU-PLUGIN: yêu cầu {expected_kind}, thực tế {component.kind}"
            )
            report.blocked_components += 1
            report.components.append(component)
            continue
