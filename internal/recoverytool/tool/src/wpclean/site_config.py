...
from .secret_runtime import load_runtime_secret
...
def load_site_profile(path: Path) -> SiteConnectionProfile:
    ...
    stored_password = str(raw["password"]) if raw.get("password") else None
    password = stored_password or load_runtime_secret(path.stem)
