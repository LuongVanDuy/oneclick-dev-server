...
def classify_exception(exc: BaseException, *, stage: str = "") -> OperatorError:
    ...
    if "wordpress" in text and "package" in text and any(
        marker in text
        for marker in ("10060", "10061", "connection refused", "actively refused", "timed out", "urlopen")
    ):
        return _error(
            "WP-CORE-DOWNLOAD-001",
            "Chưa tải được WordPress sạch",
            "Máy không kết nối được wordpress.org và chưa có package WordPress hợp lệ trong cache local.",
            "Kiểm tra kết nối Internet rồi thử lại. Sau lần tải thành công đầu tiên, các project tiếp theo sẽ dùng cache local khi mạng lỗi.",
            technical,
        )
    ...
    # FTP servers normally include status 550.  A plain PermissionError with a
    # local Windows path is instead a filesystem problem, even though Windows
    # uses the same "permission denied" wording.
    if "550" in text or (
        stage_key.startswith("ftp")
        and ("permission denied" in text or "access is denied" in text)
    ):
        return _error(
            "FTP-PERM-001",
    ...
    if isinstance(exc, PermissionError) and not stage_key.startswith("ftp"):
        return _error(
            "LOCAL-FS-PERM-001",
            "Máy không ghi được file tạm",
            "Windows từ chối quyền ghi hoặc dọn một file tạm trên máy.",
            "Ứng dụng sẽ dùng thư mục runtime bên trong dự án; thử lại đúng bước hiện tại.",
            technical,
        )
