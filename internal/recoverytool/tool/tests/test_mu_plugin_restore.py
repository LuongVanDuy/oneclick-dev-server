...
def test_clean_mu_plugin_component_is_allowed(tmp_path: Path):
    ...
    (mu / "flatsome-custom-loader.php").write_text(
...
def test_clean_but_non_allowlisted_mu_plugin_is_blocked(tmp_path: Path):
    root = tmp_path / "backup"
    mu = root / "mu-plugins"
    mu.mkdir(parents=True)
    (mu / "unknown-safe.php").write_text("<?php // harmless but not approved", encoding="utf-8")

    report = scan_mu_plugins(root)

    assert report.components[0].blocked is True
    assert "ALLOWLIST" in report.components[0].unreadable_files[0]
...
def test_one_bad_file_blocks_whole_mu_plugin_directory(tmp_path: Path, synthetic_code_samples):
    root = tmp_path / "backup"
    component = root / "mu-plugins" / "flatsome-custom-assets"
    ...
    assert item.name == "flatsome-custom-assets"
...
def test_backup_exclusion_blocks_matching_mu_component(tmp_path: Path):
    ...
    mu.mkdir(parents=True)
    (mu / "flatsome-custom-loader.php").write_text(
        "<?php // safe but incomplete backup", encoding="utf-8"
    )
        ...
        json.dumps(
            {
                "exclusions": [
                    {
                        "stage": "mu-plugins",
                        "path": "/domains/example.com/public_html/wp-content/mu-plugins/flatsome-custom-loader.php",
                        "error": "FTP reset",
                    }
                ]
            }
...
def test_run_stage_uploads_only_clean_components(tmp_path: Path, monkeypatch, synthetic_code_samples):
    ...
    mu.mkdir(parents=True)
    (mu / "flatsome-custom-loader.php").write_text(
        "<?php add_action('init', 'safe_init');", encoding="utf-8"
    )
    ...
    assert uploaded == ["/public_html/wp-content/mu-plugins/flatsome-custom-loader.php"]
    assert report.completed is True
