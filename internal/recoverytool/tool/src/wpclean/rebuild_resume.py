...
from .backup import verify_manifest_metadata
...
# Compatibility name retained for integrations/tests that patch the historical
# verifier. DB-only resume now uses the fast receipt/metadata verifier because a
# complete SHA pass already ran immediately before the destructive rebuild.
verify_manifest = verify_manifest_metadata
...
def resume_database_import(
    *,
    ...
    required_remote_state = {
        "remote_wipe": (
            bool(report.get("remote_wipe_completed"))
            # Backward compatibility: reports created before the explicit flag
            # can still prove wipe completion when core upload already started.
            or int(report.get("core_uploaded") or 0) > 0
            or int(report.get("wiped_files") or 0) > 0
            or int(report.get("wiped_dirs") or 0) > 0
        ),
