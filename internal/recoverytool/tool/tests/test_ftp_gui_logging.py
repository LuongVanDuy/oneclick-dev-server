...
def _sandbox(tmp_path: Path, monkeypatch) -> str:
    ...
    monkeypatch.setattr(
        ftp_gui,
        "_measure_worker_advice",
        lambda profile, _job: {
            "available": True,
            "suggestedWorkers": 6,
            "confirmedConnections": 8,
            "probeCap": 16,
            "reachedServerLimit": True,
            "exactLimitKnown": False,
            "reason": "Đã xác nhận an toàn trong test.",
            "stages": [],
            "host": profile.host,
            "port": profile.port,
        },
...
def test_manual_ftp_test_is_visible_in_terminal_and_persistent_history(tmp_path: Path, monkeypatch) -> None:
    ...
    assert result["workerAdvice"]["suggestedWorkers"] == 6
    ...
    assert "FTP WORKERS · khuyến nghị 6" in messages
    assert "ftp-secret" not in messages

    payload = gui_server._project_payload(name)
    assert payload["ftpWorkerAdvice"]["confirmedConnections"] == 8
...
def test_production_gui_injects_immediate_ftp_test_terminal_feedback() -> None:
    ...
    assert "GỢI Ý LUỒNG FTP" in html
    assert "applyFtpWorkerAdvice" in html
    assert "Nhấn Lưu để áp dụng" in html
