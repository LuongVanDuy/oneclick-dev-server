...
def _paths(profile: SiteConnectionProfile, profile_path: Path) -> dict[str, Path]:
    ...
    backup_root_base = BACKUPS_DIR.resolve()
    backup_root = (backup_root_base / profile.host).resolve()
    if remembered_backup:
        remembered_path = Path(str(remembered_backup))
        candidate = remembered_path.resolve() if remembered_path.is_absolute() else (backup_root_base / remembered_path.name).resolve()
        try:
            candidate.relative_to(backup_root_base)
            backup_root = candidate
        except ValueError:
            # Legacy imports may remember the absolute root of the old app.
            # Rebase only a host-owned backup directory into this workspace;
            # never follow or mutate the original location.
            legacy_name = remembered_path.name
            if legacy_name == profile.host or legacy_name.startswith(f"{profile.host}-"):
                backup_root = (backup_root_base / legacy_name).resolve()
    if remembered_backup and str(remembered_backup) != str(backup_root):
        state["backup_root"] = str(backup_root)
        state["profile"] = str(profile_path.resolve())
...
def _infer_status(paths: dict[str, Path]) -> dict[str, Any]:
    ...
    # GUI mode deliberately defers the expensive full-tree SHA pass until the
    # database has also been downloaded. The file stage is complete when every
    # required FTP transfer succeeded; backup_ready still requires the final
    # manifest produced by the database stage.
    filesystem_backup = bool(backup_report.get("verified"))
    database_backup = (
        (backup_root / "database" / "original.sql").is_file()
        and (backup_root / "manifest.json").is_file()
    )
...
def _stage_backup_files(profile: SiteConnectionProfile, transport: FTPTransport, paths: dict[str, Path]) -> None:
    ...
    with console.status("[cyan]Đang chuẩn bị backup...[/cyan]", spinner="dots") as status_ui:
        def progress(event: dict) -> None:
            ...
            elif phase == "discover_transfer":
                status_ui.update(
                    f"[cyan]Đang quét và tải {stage}: "
                    f"{event.get('files_completed', 0)}/{event.get('files_found', 0)} file đã tìm thấy[/cyan]"
                )
            elif phase == "transfer":
                status_ui.update(
