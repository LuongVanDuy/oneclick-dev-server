...
from .secret_runtime import remember_runtime_secret
...
def _json_write(path: Path, payload: dict[str, Any]) -> None:
    if os.environ.get("WPCLEAN_EMBEDDED") == "1":
        try:
            if path.parent.resolve() == SITES_DIR.resolve():
...
def create_project(data: dict[str, Any]) -> dict[str, Any]:
    ...
    SITES_DIR.mkdir(parents=True, exist_ok=True)
    remember_runtime_secret(project_name, password)
...
_REBUILD_PROGRESS_FIXED = {
    "verify_integrity_checkpoint": 1,
    "integrity_checkpoint_reused": 4,
    "verify_original": 1,
    "verify_clean": 3,
    "download_core": 5,
    "extract_core": 8,
...
def _progress(job: GuiJob, event: dict[str, Any]) -> None:
    ...
    bytes_completed = number("bytes_completed", "bytes_downloaded", fallback=job.bytes_completed)
    ...
    if phase in {"manifest_inventory", "manifest_build", "manifest_verify"} and job.stage != "rebuild":
        pass_index = max(1, number("pass_index", fallback=1))
        pass_total = max(pass_index, number("pass_total", fallback=2))
        within_pass = min(1.0, completed / total) if total > 0 else (
            min(1.0, bytes_completed / bytes_total) if bytes_total > 0 else 0.0
        )
        manifest_ratio = min(1.0, ((pass_index - 1) + within_pass) / pass_total)
        if job.stage in {"backup-db", "clean"}:
            percent = round(70 + (30 * manifest_ratio))
        else:
            percent = round(100 * manifest_ratio)
    elif job.stage == "verify-scan" and phase == "security_scan_database":
        ratio = min(1.0, bytes_completed / bytes_total) if bytes_total else 0.0
        percent = round(5 + (30 * ratio))
    elif job.stage == "verify-scan" and phase == "security_scan_uploads":
        ratio = min(1.0, completed / total) if total else 0.0
        percent = round(35 + (60 * ratio))
    elif job.stage == "clean" and phase == "security_scan_uploads":
        ratio = min(1.0, completed / total) if total else 0.0
        percent = round(5 + (30 * ratio))
    elif job.stage == "clean" and phase == "clean_uploads_plan":
        ratio = min(1.0, completed / total) if total else 0.0
        percent = round(35 + (15 * ratio))
    elif job.stage == "clean" and phase == "clean_database":
        ratio = min(1.0, bytes_completed / bytes_total) if bytes_total else 0.0
        percent = round(50 + (20 * ratio))
    elif job.stage == "plugin" and plugin_total:
    ...
    elif job.stage == "rebuild" and phase in {"manifest_inventory", "manifest_build", "manifest_verify"}:
        percent = 1 if number("pass_index", fallback=1) <= 1 else 3
    elif job.stage == "rebuild" and phase and event.get("discovery_complete") is False:
        # Remote deletion and local upload now begin while their inventories are
        # still growing. Keep the stage bar monotonic until the denominator is final.
        bounds = _REBUILD_PROGRESS_RANGES.get(phase)
        percent = max(job.percent, bounds[0] if bounds else job.percent)
    elif job.stage == "rebuild" and phase:
        percent = _rebuild_progress_percent(phase, completed, total, job.percent)
    elif phase == "discover_transfer" and event.get("discovery_complete") is False:
        # The denominator is still growing while FTP inventory and downloads
        # overlap.  Keep the bar monotonic instead of briefly showing 100% and
        # moving backwards as more files are discovered.
        percent = job.percent
    elif total:
    ...
    labels = {
        ...
        "discover_transfer": "Đang quét và tải backup",
        "discovered": "Đã kiểm kê xong, tiếp tục tải",
        ...
        "ftp_download_path_fallback": "FTP đang đổi cách tải file",
        "reconcile": "Đang tải lại các file FTP còn thiếu",
        "verify": "Đang xác minh SHA-256",
        "manifest_inventory": "Đang lập danh sách file local",
        "manifest_build": "Đang tạo SHA-256 song song",
        "manifest_verify": "Đang xác minh SHA-256 song song",
        "resource_wait": "Đang xếp hàng dùng ổ đĩa",
        "resource_acquired": "Đã bắt đầu xử lý trên máy",
        "clean_uploads": "Đang tạo uploads sạch",
        "clean_uploads_plan": "Đang lập danh sách uploads an toàn",
        "clean_database": "Đang làm sạch database trên máy",
        "security_scan_database": "Đang quét database trên máy",
        "security_scan_uploads": "Đang quét uploads trên máy",
        ...
        "db_export_bootstrap_repair": "Đang phục hồi PHP để xuất database",
        "db_export_execute": "Đang tạo file database theo checkpoint",
        "db_export_retry": "HTTP tạm ngắt, đang đọc checkpoint FTP",
        "db_export_download": "Đang tải database về máy",
        "remove_bridge": "Đang dọn file tạm",
        "mu_plugin_prune": "Đang xóa MU-plugin ngoài allowlist",
        "verify_original": "Đang xác minh backup gốc",
        "verify_clean": "Đang xác minh dữ liệu sạch",
        "verify_integrity_checkpoint": "Đang kiểm tra checkpoint SHA-256",
        "integrity_checkpoint_reused": "SHA-256 không đổi, tiếp tục từ checkpoint",
        "download_core": "Đang tải WordPress sạch",
        "download_core_retry": "Không tải được WordPress, đang thử lại",
        "download_core_cache": "Đang dùng WordPress sạch từ cache local",
        "download_core_cache_fallback": "Mạng lỗi, đang dùng WordPress sạch từ cache local",
    ...
    error_detail = str(event.get("error") or "").replace("\r", " ").replace("\n", " ").strip()
    for secret in getattr(job, "_journal_secrets", ()):
        value = str(secret or "")
        if len(value) >= 3:
            error_detail = error_detail.replace(value, "***")
    if error_detail and phase in {
        "retry",
        "file_failed",
        "ftp_download_path_fallback",
        "stage_failed",
        "config_file",
    }:
        message += f" · {error_detail[:500]}"
    ...
    try:
        ...
        elif job.bytes_per_second > 0 and job.bytes_total >= job.bytes_completed:
            job.eta_seconds = int((job.bytes_total - job.bytes_completed) / job.bytes_per_second)
    except (TypeError, ValueError):
        pass
    inferred_location = ""
    if phase.startswith(("manifest_", "clean_", "security_scan_", "resource_")):
        inferred_location = "local"
    elif phase in {
        "discover", "discover_transfer", "discovered", "transfer", "retry", "reconcile", "file_failed",
        "stage", "config_file", "wipe", "wipe_inventory", "wipe_inventory_complete",
        "ftp_reconnect", "ftp_path_fallback", "ftp_download_path_fallback", "permission_recovery", "db_import_execute",
        "db_import_retry", "upload_bridge", "request_dump", "db_export_execute",
    } or phase.startswith("upload_"):
        inferred_location = "hosting"
    job.work_location = str(event.get("location") or inferred_location or job.work_location or "")
    job.pass_index = number("pass_index", fallback=job.pass_index)
    job.pass_total = number("pass_total", fallback=job.pass_total)
    job.worker_count = number("workers", fallback=job.worker_count)
...
def _run_backup_files(profile, transport, paths, job: GuiJob) -> None:
    ...
    if report.removed_remote_mu_plugins:
        job.log(
            f"Đã xóa {len(report.removed_remote_mu_plugins)} MU-plugin ngoài allowlist: "
            + ", ".join(report.removed_remote_mu_plugins[:20])
        )
...
def _run_backup_db(profile, transport, paths, job: GuiJob) -> None:
    ...
    result = export_database_via_php_bridge(
        ...
        target,
        progress=lambda event: _progress(job, event),
        bootstrap_config_dir=paths["backup"] / "config",
    )
    with disk_heavy(lambda event: _progress(job, event), operation="backup-manifest"):
    ...
    if result.bootstrap_placeholders:
        job.log(
            "Đã tạo placeholder PHP an toàn cho cấu hình bootstrap bị thiếu: "
            + ", ".join(result.bootstrap_placeholders)
        )


def _run_verify_scan(profile, paths, job: GuiJob) -> None:
    backup_root = paths["backup"]
    ok, problems = verify_manifest_metadata(backup_root, backup_root / "manifest.json")
    if not ok:
    ...
    with disk_heavy(lambda event: _progress(job, event), operation="security-scan"):
        db_findings = scan_sql(sql_path, progress=lambda event: _progress(job, event)) if sql_path.is_file() else []
        job.set(message="Đã quét database, đang quét uploads", percent=65, current=str(uploads_path))
        upload_findings = (
            scan_uploads(uploads_path, progress=lambda event: _progress(job, event))
...
def _run_clean(profile, paths, job: GuiJob) -> None:
    ...
    with disk_heavy(lambda event: _progress(job, event), operation="clean-staging"):
        report = build_clean_restore(
            paths["backup"],
            ftp_password=profile.password or "",
            host=profile.host,
            progress=lambda event: _progress(job, event),
                    ...
                    if str(options.get("confirmRebuild") or "") != profile.host:
                        _gate(
                            ...
                            (
                                ...
                                "Muốn nhanh hơn, có thể dùng File Manager của hosting xóa trước nội dung bên trong đúng thư mục này; "
                                "giữ .well-known, không xóa chính thư mục gốc và không thao tác đồng thời sau khi app đã bắt đầu rebuild."
                            ),
...
def _send_html(handler: BaseHTTPRequestHandler, html: str) -> None:
    ...
    handler.send_header("Content-Length", str(len(data)))
    handler.send_header("Cache-Control", "no-store")
    # Stable machine-readable identity for OneClick. Keep this independent
    # from visible product copy so wording/theme changes cannot break startup.
    handler.send_header("X-OneClick-Recovery", "1")
...
class GuiHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        try:
            if path == "/":
                ...
                return
            if path == "/fresh":
                from .fresh_ui import render_fresh_app
                _send_html(self, render_fresh_app(TOKEN))
                return
            if path == "/api/fresh-installs":
                from .fresh_install import list_installs
                _send_json(self, {"installs": list_installs()})
                return
            match = re.fullmatch(r"/api/fresh-installs/([^/]+)", path)
            if match:
                from .fresh_install import get_install
                _send_json(self, get_install(unquote(match.group(1))))
                return
    ...
    def do_POST(self) -> None:  # noqa: N802
        parsed = urlparse(self.path)
        path = parsed.path.rstrip("/") or "/"
        control_token = os.environ.get("WPCLEAN_CONTROL_TOKEN") or ""
        if path == "/api/oneclick/shutdown":
            supplied = self.headers.get("X-OneClick-Control") or ""
            if os.environ.get("WPCLEAN_EMBEDDED") != "1" or not control_token or not secrets.compare_digest(supplied, control_token):
                _send_json(self, {"error": "Phiên điều khiển OneClick không hợp lệ."}, 403)
                return
        ...
        try:
            ...
            if path == "/api/fresh-installs/probe-workers":
                from .fresh_install import probe_install_workers
                _send_json(self, probe_install_workers(body))
                return
            if path == "/api/fresh-installs":
                from .fresh_install import create_install
                _send_json(self, create_install(body), 201)
                return
            match = re.fullmatch(r"/api/fresh-installs/([^/]+)/delete", path)
            if match:
                from .fresh_install import delete_install
                _send_json(
                    self,
                    delete_install(unquote(match.group(1)), str(body.get("confirmation") or "")),
                )
                return
            match = re.fullmatch(r"/api/fresh-installs/([^/]+)", path)
            if match:
                from .fresh_install import update_install
                _send_json(self, update_install(unquote(match.group(1)), body))
                return
            match = re.fullmatch(r"/api/fresh-installs/([^/]+)/test", path)
            if match:
                from .fresh_install import test_install_connection
                _send_json(self, test_install_connection(unquote(match.group(1))))
                return
            match = re.fullmatch(r"/api/fresh-installs/([^/]+)/start", path)
            if match:
                from .fresh_install import start_install
                _send_json(self, start_install(unquote(match.group(1))), 202)
                return
            if path == "/api/projects":
...
def _bind_server(host: str = "127.0.0.1", preferred: int = 8765) -> tuple[ThreadingHTTPServer, int]:
    ...
    attempts = 1 if os.environ.get("WPCLEAN_EMBEDDED") == "1" else 20
...
def main() -> None:
    preferred = int(os.environ.get("WPCLEAN_PORT") or "8765")
    server, port = _bind_server(preferred=preferred)
