...
from .backup import verify_manifest, verify_manifest_metadata
...
def run_rebuild_preflight(
    *,
    ...
    verifier = verify_manifest_metadata if fast else verify_manifest
    original_ok, original_problems = verifier(backup_root, original_manifest)
    if not original_ok:
    ...
    clean_ok, clean_problems = verifier(clean_root, clean_manifest)
    if not clean_ok:
