...
from . import gui_ui
from .secret_runtime import forget_runtime_secret
...
renderDrawer=function(p){
  ...
  const deleting=!!p.localDeletion?.status,active=p.job?.status==='running';
  qs('#drawerContent').innerHTML=`<div class="drawer-head"><div class="drawer-title"><div><h2>${esc(p.host)}</h2><p>${esc(p.name)} · ${pc}% hoàn tất</p></div><button class="xbtn" type="button" aria-label="Đóng chi tiết dự án" onclick="closePanels()">×</button></div></div><div class="drawer-body"><div class="detail-layout"><div class="detail-left"><div class="section"><div class="section-head"><div class="section-title">Kết nối FTP</div></div>${connectionHtml(p)}</div><div class="section"><div class="section-head"><div class="section-title">Tiến độ xử lý</div></div><div class="steps">${stepHtml(p)}</div>${jobHtml(p)}${decisionHtml(p)}</div>${p.themeRepair&&!deleting?`<div class="section"><button class="btn btn-warning" onclick="openRepair('${esc(p.name)}')">Mở thư mục theme repair</button></div>`:''}${!deleting?`<div class="foot-danger"><p>Chỉ xóa dữ liệu trên máy này; website trên hosting không thay đổi.</p><button class="btn btn-danger" ${active?'disabled title="Hãy chờ bước đang chạy kết thúc"':''} onclick="deleteProject('${esc(p.name)}')">Xóa dự án local</button></div>`:''}</div><div class="detail-right">${terminalPanelHtml(p)}</div></div></div>`;
    ...
    if isinstance(deletion, dict) and deletion.get("status") == "running":
        with _LOCAL_DELETE_LOCK:
            ...
            if thread is None or not thread.is_alive():
                ...
                deletion_changed = True
    if isinstance(deletion, dict) and deletion:
        from . import project_delete_command as delete_module

        stored_targets = deletion.get("targets")
        stored_targets = stored_targets if isinstance(stored_targets, dict) else {}
        target_specs = {
            "backup": (paths["backup"], server.BACKUPS_DIR),
            "reports": (server.REPORTS_DIR / profile.host, server.REPORTS_DIR),
            "repairs": (server.REPAIRS_DIR / profile.host, server.REPAIRS_DIR),
        }
        safe_targets: dict[str, str] = {}
        rebased = False
        for key, (fallback, allowed_root) in target_specs.items():
            raw_target = stored_targets.get(key)
            candidate = Path(str(raw_target)) if raw_target else fallback
            if not delete_module._is_within(candidate, allowed_root):
                candidate = fallback
                rebased = True
            safe_targets[key] = str(candidate)
        if rebased:
            deletion = {
                **deletion,
                "status": "interrupted",
                "message": "Đã chuyển đường dẫn cũ sang workspace OneClick. Có thể tiếp tục xóa an toàn.",
                "error": "",
                "technical": "",
...
def _update_project(name: str, data: dict[str, Any]) -> dict[str, Any]:
    ...
    password = str(data.get("password") or profile.password or "")
    ...
    server.remember_runtime_secret(name, password)
    server._json_write(profile_path, raw)
...
def _delete_project_local(name: str, confirmation: str) -> dict[str, Any]:
    ...
    with _LOCAL_DELETE_LOCK:
        ...
        def local_target(key: str, fallback: Path, allowed_root: Path) -> Path:
            from . import project_delete_command as delete_module
            ...
            if not delete_module._is_within(candidate, allowed_root):
                candidate = fallback
            if not delete_module._is_within(candidate, allowed_root):
                raise RuntimeError(f"Đường dẫn local của {key} không nằm trong workspace OneClick.")
            return candidate
        ...
        targets = {
            ...
            "backup": local_target("backup", paths["backup"], server.BACKUPS_DIR),
            "reports": local_target("reports", server.REPORTS_DIR / profile.host, server.REPORTS_DIR),
            "repairs": local_target("repairs", server.REPAIRS_DIR / profile.host, server.REPAIRS_DIR),
        }
