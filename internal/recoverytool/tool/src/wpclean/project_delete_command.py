...
def _native_delete_path(path: Path) -> str:
...
def _retryable_delete_error(exc: OSError) -> bool:
    return getattr(exc, "winerror", None) in {5, 32, 145} or exc.errno in {
        errno.EACCES,
        errno.EBUSY,
        errno.ENOTEMPTY,
    }
...
def _delete_local_path(path: Path, allowed_root: Path) -> bool:
    ...
    native_path = _native_delete_path(path)
    if path.is_dir() and not path.is_symlink():
        last_error: OSError | None = None
        for attempt in range(DELETE_ATTEMPTS):
            try:
                shutil.rmtree(native_path, onerror=_on_remove_error)
            except OSError as exc:
                last_error = exc
                if not _retryable_delete_error(exc) or attempt == DELETE_ATTEMPTS - 1:
                    raise
                time.sleep(min(0.1 * (2**attempt), 1.0))
                continue
            if not path.exists() and not path.is_symlink():
                return True
            last_error = OSError(errno.ENOTEMPTY, f"Thư mục vẫn còn dữ liệu sau lượt xóa {attempt + 1}: {path}")
            time.sleep(min(0.1 * (2**attempt), 1.0))
